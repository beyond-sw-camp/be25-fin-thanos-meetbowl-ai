...
class Settings(BaseSettings):
    ...
    rerank_top_n: int = 10
    # 챗봇 검색 노이즈 차단: 리랭크 관련도(0~1)가 이 값 미만이면 출처에서 제외한다(0이면 비활성).
    # 벡터/RRF 점수 스케일은 모델·컬렉션마다 달라 0.75 같은 높은 고정값은 좋은 근거도 자를 수 있다.
    ...
    # agentic 툴 루프 모델 호출 상한. 날짜/건수/열거형 복합 질의는 검색 후 최종 답변까지
    # 4회 이상 모델 요청이 필요할 수 있어 5를 기본으로 둔다.
    chat_request_limit: int = 5
